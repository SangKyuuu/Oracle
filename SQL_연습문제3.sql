...
//3-15
SELECT SUM(TR_AMOUNT) 임금총합, AVG(TR_AMOUNT) 임금평균 FROM TRANSACTION WHERE TR_TYPE = 1;

//3-16
SELECT MAX(TR_AMOUNT) "큰 금액", MIN(TR_AMOUNT) "작은 금액" FROM TRANSACTION WHERE TR_TYPE = 2;

//3-17
SELECT * FROM ACCOUNT WHERE ACC_NAME = '자유저축예금' AND ACC_BALANCE >= 1000000 ORDER BY ACC_BALANCE DESC;

//3-18
SELECT * FROM ACCOUNT WHERE ACC_NAME = '자유저축예금' ORDER BY ACC_BALANCE DESC FETCH FIRST 1 ROW ONLY;

//3-19
SELECT * FROM TRANSACTION WHERE TR_TYPE <> 0 ORDER BY TR_TYPE, TR_AMOUNT DESC;
