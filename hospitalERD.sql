...
insert into doctor values('D101102','101','계백','1975-06-11','M','과장','010-1102-1975','gaeback@bw.com');
insert into doctor values('D101103','101','김관창','1989-05-30','M','과장','010-1103-1989','kwanch@bw.com');
insert into doctor values('D102101','102','김춘추','1979-04-13','M','과장','010-2101-1979','kimcc@bw.com');
insert into doctor values('D102104','102','이사부','1966-09-12','M','과장','010-2104-1966','leesabu@bw.com');
...
insert into doctor values('D107103','107','이이','1992-09-07','M','과장','010-7103-1992','leelee@bw.com');
insert into doctor values('D107104','107','이황','1989-12-09','M','과장','010-7104-1989','hwang@bw.com');
...
select
    ...
    dpm.dep_name
from doctor d
join department dpm on d.dep_no = dpm.dep_no;
...
join patient p on t.pat_id = p.pat_id
join chart c on t.treat_no = c.treat_no
where d.dep_no = (select dep_no from department where dep_name ='외과');
...
WHERE FLOOR(MONTHS_BETWEEN(SYSDATE, 
    TO_DATE(
        CASE 
            WHEN SUBSTR(pat_jumin, 8, 1) IN ('1', '2') 
            THEN '19' || SUBSTR(pat_jumin, 1, 6)
            WHEN SUBSTR(pat_jumin, 8, 1) IN ('3', '4') 
            THEN '20' || SUBSTR(pat_jumin, 1, 6)
        END, 'YYYYMMDD'
    )
) / 12) BETWEEN 30 AND 39;
...
select 
    ...
    dpm.dep_name
from doctor d
right join department dpm on d.dep_no = dpm.dep_no
